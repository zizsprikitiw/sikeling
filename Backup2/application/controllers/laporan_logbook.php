...
class Laporan_logbook extends CI_Controller {
	...
	{						
		...
		$this->_render_page('laporan_logbook', $this->data);				
	}		
	...
	{				
		...
		$where =  array('user_id' => $this->session->userdata('user_id'), 'status' => $this->jenis_laporan, 'EXTRACT(year FROM "tgl_dokumen")=' => $_POST['filter_tahun']);  //laporan logbook (status=1)
