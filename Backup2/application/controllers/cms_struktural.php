...
class Cms_struktural extends CI_Controller {
	...
	{																	
		...
		$this->_render_page('cms/cms_struktural', $this->data);				
	}		
	...
	{		
		...
		foreach ($list as $list_item) {
			...
			//add html for action
			$row[] = '<a class="btn btn-xs btn-primary" href="javascript:void()" title="Menu" onclick="struktural_menu('."'".$list_item->id."','".$list_item->nama."'".')"><i class="fa fa-sitemap"></i></a>
	...
	public function menu_list()
	{
		...
		foreach($main as $main_item){
			...
			if($struktural_menu_id != 0){
				$row[] = '<button class="btn btn-xs btn-success" onclick="data_edit_status('."'".$main_item->id."','".$struktural_menu_id."'".')">On</button>';
			}else{
			...
			for($j=1; $j<=$sub_len; $j++){
				if($main_item->id == $sub[$j][1]->ref_id){
					...
					foreach($sub[$j] as $sub_item){
						...
						if($struktural_menu_id != 0){
							$row[] = '<button class="btn btn-xs btn-success" onclick="data_edit_status('."'".$sub_item->id."','".$struktural_menu_id."'".')">On</button>';
						}else{
