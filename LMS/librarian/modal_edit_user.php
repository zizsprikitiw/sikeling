	...
	<form class="form-horizontal" method="post">
			...
			<div class="control-group">
				...
				<div class="controls">
					<select name="status">
						<option value="<?php echo $classname; ?>"><?php echo $row['status']; ?></option>
						<?php $query1 = mysql_query("select * from status_login where status_id  != '$classname'")or die(mysql_error());
						while($row1 = mysql_fetch_array($query1)){
						?>
						<option value="<?php echo $row1['classname']; ?>"><?php echo $row1['classname']; ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
	...
	mysql_query("update users set username='$username', password='$password' , firstname = '$firstname' , lastname = '$lastname' , status = '$status'  where user_id='$user_id'")or die(mysql_error()); ?>
	<script>
