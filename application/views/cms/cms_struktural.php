  	...
  	<div class="mainbar" >      
	    ...
	    <div class="matter" >
			<div class="container" >								
				...
				<button class="btn btn-success" onClick="data_add()"><i class="fa fa-plus"></i> Tambah Struktural</button>
				<br />
				<br />
				<!-- Table Page -->
						...
						<table class="table-hover table-bordered" cellpadding="0" cellspacing="0" border="0" id="table" width="100%">
							<thead style="background-color:#006699; color:#FFFFFF;">
								<tr>
								  ...
								  <th ><b>Singkatan</b></th>
								  <th width="120px" style="max-width:120px"><b>Aksi</b></th>
								</tr>
							</thead>													
						</table>						
						<div class="clearfix"></div>									
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {																
			 	table = $('#table').DataTable({ 			
					...
					"ajax": {
						"url": "<?php echo site_url('cms_struktural/data_list')?>",
						"type": "POST"
					},
				...
				table_menu = $('#table_menus').DataTable({ 			
					...
					// Load data for the table's content from an Ajax source
					"ajax": {
						"url": "<?php echo site_url('cms_struktural/menu_list/')?>",
						"type": "POST",
						"data":function ( d ) {
			...
			function data_edit(id)
			{
			  ...
			  $.ajax({			  		
					url : "<?php echo site_url('cms_struktural/data_edit/')?>/" + id,
					type: "GET",
					dataType: "JSON",
			...
			function data_save()
			{
			  ...
			  if(save_method == 'add') 
			  {
				  url = "<?php echo site_url('cms_struktural/data_save_add')?>";
			  }
			  else
			  {
				url = "<?php echo site_url('cms_struktural/data_save_edit')?>";
			  }
			...
			function data_delete(id, nama_id)
			{
				if(id != ''){
					...
					$.ajax({
						url : "<?php echo site_url('cms_struktural/data_delete')?>/",
						type: "POST",
			...
			function data_edit_status(menu_id, struktural_menu_id)
			{
				...
				$.ajax({
						url : "<?php echo site_url('cms_struktural/data_edit_status')?>",
						type: "POST",
						dataType: "JSON",
						data: form_data,
						success: function(data)
