  	...
  	<div class="mainbar" >      
		...
		<script type="text/javascript">		
			...
			function load_select_filter(){
				 $.ajax({
					url : "<?php echo site_url('laporan_logbook_in/data_init/')?>" ,
					type: "POST",
					dataType: "JSON",
			...
			$(document).ready(function() {								  			
				...
				$("#modalApproval").bind("hidden.bs.modal", function(e){
					...
					$.ajax({
							url : "<?php echo site_url('laporan_logbook_in/show_laporan_delete')?>",
							type: "POST",
							dataType: "JSON",
							data: form_data,
							success: function(data)
				...
				//load data table																
			 	table = $('#table').DataTable({ 			
					...
					"ajax": {
						"url": "<?php echo site_url('laporan_logbook_in/data_list')?>",
						"type": "POST",						
						"data": function ( d ) {
								var chkSearch = [];
			...
			{				 				
				...
				$.ajax({
					url : "<?php echo site_url('laporan_logbook_in/show_laporan')?>",
					type: "POST",
					dataType: "JSON",
					data: form_data,
					success: function(data)
			...
			function approval_laporan(id, nama, judul, logbook_id, user_id_penerima)
			{
				if(logbook_id != ''){
					...
					$('[name="logbook_id"]').val(logbook_id);
					$('[name="user_id_penerima"]').val(user_id_penerima);
					...
					$.ajax({
						url : "<?php echo site_url('laporan_logbook_in/approval_laporan')?>/",
						type: "POST",
				...
				<div class="modal-content" >														
					  ...
					  <div class="modal-body" >
						...
						<form class="form-horizontal" role="form" id="approve_form" action="#">
					  		...
					  		<input type="hidden" value="" name="logbook_id"/> 									
					  		<input type="hidden" value="" name="user_id_penerima"/> 									
