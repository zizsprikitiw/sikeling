...
class Cms_konfigurasi extends CI_Controller {
	
	//Global variable
	var $table_name = 'config_proyek';
	...
	{																
		...
		$this->_render_page('cms/cms_konfigurasi', $this->data);				
	}		
	
	public function data_list()
	{		
		$search_column = array('parameter_name');
		$search_order = array('parameter_name' => 'desc');
		...
		foreach ($list as $list_item) {
			...
			$row = array();
			$row[] = $no;
			$row[] = $list_item->parameter_name;
			$row[] = $list_item->parameter_value;			
			...
			$row[] = '<a class="btn btn-xs btn-warning" href="javascript:void()" title="Edit" onclick="data_edit('."'".$list_item->id."'".')"><i class="fa fa-pencil"></i></a>
				  <a class="btn btn-xs btn-danger" href="javascript:void()" title="Hapus" onclick="data_delete('."'".$list_item->id."','".$list_item->parameter_name."'".')"><i class="fa fa-times"></i></a>';
	...
	{					
		...
		$this->form_validation->set_rules('parameter_name', 'Nama Parameter', 'trim|required|is_unique[' . $this->table_name . '.parameter_name]');
		$this->form_validation->set_rules('parameter_value', 'Nilai', 'trim|required');		
	...
	{				
		...
		//if($data_list->parameter_name != $parameter_name){
		//	$this->form_validation->set_rules('parameter_name', 'Nama Parameter', 'trim|required|is_unique[' . $this->table_name . '.parameter_name]');
		//}				
		$this->form_validation->set_rules('parameter_value', 'Nilai', 'trim|required');		
