...
class Laporan_perjadin_masuk extends CI_Controller {
	...
	{						
		...
		$this->_render_page('laporan_perjadin_masuk', $this->data);				
	}		
	...
	{	
		...
		foreach ($list as $list_item) {
			...
			if(! is_null($list_item->nama_group_pengirim)){
			...
			if(is_null($list_item->approval_date)){
				$row[] = '<a class="btn btn-xs btn-warning" href="javascript:void()" title="Approval" align="center" onclick="approval_laporan('."'".$list_item->id."','".$pengirim."','".$list_item->filename."','".$list_item->log_book_id."'".')"><i class="fa fa-check"></i> Approve</a>';			
			}else{
