...
class Laporan_keluar extends CI_Controller {
	...
	{						
		...
		$this->_render_page('laporan_keluar', $this->data);				
	}		
	...
	{	
		...
		foreach ($list as $list_item) {
			...
			if(! is_null($list_item->nama_user_penerima)){
				$penerima = $list_item->nama_user_penerima;				
				
				if(! is_null($list_item->nama_struktural_penerima)){
					$jabatan = $list_item->nama_struktural_penerima;
				}else{
					$jabatan = $list_item->nama_posisi_penerima;
					
					if(! is_null($list_item->groups_leader_nama_penerima)){
						$jabatan = $jabatan.' '.$list_item->groups_leader_nama_penerima.' ('.$list_item->groups_leader_singkatan_penerima.')';
					}elseif(! is_null($list_item->leader_nama_penerima)){
						$jabatan = $jabatan.' '.$list_item->leader_nama_penerima.' ('.$list_item->leader_singkatan_penerima.')';
					}
				}				
			}else{
				$penerima = $list_item->nama_user_penerima_struktural;
				$jabatan = $list_item->nama_struktural_penerima_struktural;
			}
	...
	{			
		$note_out = $this->cms_model->row_get_by_id($id, $this->table);
		//rubah status
		
		if($note_out->file_akses == 1){
			$status = 0;
		}else{
			$status = 1;
		}				
		
		if($this->cms_model->update(array("id" => $id), array('file_akses' => $status), $this->table))
		{
			//berhasil update
