...
class Laporan_perjadin extends CI_Controller {
	
	//Global variable
	var $table_name = 'log_book';
	var $view_name = 'v_users_log_book';
	var $jenis_laporan = '4';	//laporan perjalanan dinas
	var $approval_var = 'laporan_perjadin';
	...
	{						
		...
		$this->_render_page('laporan_perjadin', $this->data);				
	}		
	...
	{				
		...
		$list = $this->cms_model->get_datatables_where($this->view_name, $search_column, $search_order, $where, $order_by);
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $list_item) {
			...
			$row[] = $list_item->klasifikasi_laporan;
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->cms_model->count_all_where($this->view_name, $where),
						"recordsFiltered" => $this->cms_model->count_filtered_where($this->view_name, $search_column, $search_order, $where, $order_by),
						"data" => $data,						
				);
	...
	public function data_add()
	{
        ...
        $data['klasifikasi_laporan'] = $this->cms_model->get_klasifikasi_laporan();    				
		echo json_encode($data);
	}
	...
	{								      				
		...
		$this->form_validation->set_rules('klasifikasi_laporan', 'Klasifikasi Laporan', "required"); 
	...
	function data_save_logbook(){
		...
		if(is_file($upload_file))
		{
			...
			$additional_data = array(
					...
					'bulan' => $bulan,
					'tahun' => $tahun,
					'id_klasifikasi_laporan' => $this->input->post('klasifikasi_laporan')
					);	
