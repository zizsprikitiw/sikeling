	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				//get struktural
				$query = $this->db->query("select us.*, st.nama from users_struktural us join struktural st on us.struktural_id=st.id where (us.tahun_akhir=0 or us.tahun_akhir is null) and us.users_id=".$user->id." limit 1");
				if ($query->num_rows > 0){	
					$list = $query->result();
					
					$user->struktural_name = $list[0]->nama;
					$user->struktural_id = $list[0]->struktural_id;
				}else{
					$user->struktural_id = '0';
					$user->struktural_name = '';
				}
				
				//get fungsional
				$query = $this->db->query("select uf.*, fn.nama from users_fungsional uf join fungsional fn on uf.fungsional_id=fn.id where (uf.tahun_akhir=0 or uf.tahun_akhir is null) and uf.users_id=".$user->id." limit 1");
				if ($query->num_rows > 0){	
					$list = $query->result();
					
					$user->fungsional_name = $list[0]->nama;
					$user->fungsional_id = $list[0]->fungsional_id;
				}else{
					$user->fungsional_id = '0';
					$user->fungsional_name = '';
				}				
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
			...
			'posisi_name'    	   => $user->posisi_name,  //TAMBAHAN
			'struktural_id'		   => $user->struktural_id,  //TAMBAHAN
			'struktural_name'	   => $user->struktural_name,  //TAMBAHAN
			'fungsional_id'		   => $user->fungsional_id,  //TAMBAHAN
			'fungsional_name'	   => $user->fungsional_name  //TAMBAHAN
		);
